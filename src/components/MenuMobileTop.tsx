			...
			<>
				<nav className="text-blue text-3xl">
					<ul className="">
						...
						<li className="cursor-pointer hover:bg-gray-700 py-2">
							<span ref={refService}>Service ▽</span>

							<ul className={`flex flex-col pl-4 text-xl  py-2 block`}>
									...
									obj.activeService ? (
										...
										>
											<Link key={`navLiLink${index}`} href={obj.url} legacyBehavior passHref>
												<span key={`navLidLinkSpan${index}`} onClick={hideMenu}>
													{obj.name}
												</span>
											</Link>
										</li>
						...
						{navConfig.mainNav.map((obj, index) => (
							<li key={`navLi2${index}`} className="cursor-pointer  py-2">
								<Link key={`navLiLink2 ${index}`} href={obj.url.toString()} legacyBehavior passHref>
									<span key={`navLinkSpan2${index}`} onClick={hideMenu}>
										{obj.menuName}
									</span>
								</Link>
							</li>
	...
	return (
		<>
			...
			<div className="absolute right-0 top-5 flex justify-end pr-3 desktop:hidden">
				<Button className={`text-3xl font-bold text-blue_dark `} onClick={showMenu}>
					☰
				</Button>
			</div>
		</>
