...
import { MailIcon, ArrowUp, PhoneIcon } from "lucide-react"
...
export default function PageFooter({ showTop = true }: PageFooterProps) {
	...
	return (
        ...
        <div className="flex-1 bg-orange_light  footer-scalap" />
				...
				<div className="pt-20 z-20 w-auto min-h-[300px] bg-gradient-to-b from-white to-orange_light">
					<div className="container flex-1 flex justify-stretch items-start pb-6">
						...
						<div className="flex-1 flex flex-col items-center gap-y-2">
							{/* FOOTER LOGO*/}
							<Link href="/" className="hidden desktop:flex" aria-label="Orange Soft Wash Home">
								<Image
							...
							<div className="hidden desktop:flex justify-center">
								...
								{/* FACEBOOK BUTTON */}
								<a
									href="https://www.facebook.com/profile.php?id=61570206387732#"
									aria-label="Facebook"
									target="_blank"
									rel="noopener noreferrer"
								>
								...
								{/* NEXTDOOR BUTTON */}
								<a
									href="https://nextdoor.com/pages/orange-soft-washing-westerville-oh/"
									aria-label="Facebook"
									target="_blank"
									rel="noopener noreferrer"
								>
							...
							<Link
								href="/#Locations"
								className="hidden desktop:flex text-blue font-bold text-lg hover:scale-105"
							...
							<div className="flex text-sm text-center text-blue font-semibold">
								&copy; 2025 Orange Soft Wash, LLC.
								<br />
								All rights reserved.
							</div>
						</div>
							...
							<div className="flex-1 flex flex-col ml-6 space-y-2 ">
								{servicesConfig.OSW_Services.map((obj, index) =>
									obj.activeService ? (
										<Link
											href={obj.url}
											key={`MenuFooterServiceLink${index}`}
											className="hover:scale-105"
											aria-label={obj.aria_label}
										>
											{obj.name}
										</Link>
									) : null
						...
						<div className="hidden desktop:flex flex-1 flex-col items-start space-y-4 text-blue  border-l-2 border-l-blue font-bold mt-10 ">
							...
							<div className="flex items-center pl-6">
								{/* Mail BUTTON */}
								<ButtonMedia
									strURL={`mailto:${siteConfig.OSW_Email_Sales}`}
									aria_label="Contact sales by email"
								>
				...
				<div
					ref={topRef}
					className="hidden  fixed bottom-20 desktop:bottom-4
       right-1 p-3 z-[9990]"
				>
					<Button
