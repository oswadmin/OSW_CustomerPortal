...
const singleServiceSchema = z.enum([
	"Surfaces",
	"Siding",
	"Decks",
	"Fences",
...
const custServicesSchema = z.union([
	singleServiceSchema,
...
const requestFormSchema = z.object({
	...
	custContactType: z.enum(["Residential", "Commercial"]),
	custReferral: z
		.enum([
			"Angie",
			"Business Card",
...
export async function FormAction(formData: FormData) {
	...
	const fData = Array.from(formData.entries()).reduce<Record<string, string | string[]>>((acc, [key, value]) => {
		if (!acc[key]) {
			acc[key] = formData.getAll(key).length > 1 ? (formData.getAll(key) as string[]) : (value as string)
		}
		return acc
	}, {})
	//console.log("fData:")
